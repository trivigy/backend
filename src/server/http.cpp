...
void server::Http::on_read(error_code code) {
    ...
    request_handler(_root.string(), move(_req), _queue);
...
void server::Http::request_handler(
    ...
    auto path_x = fs::canonical(fs::path("index.html"));

    cerr << "absolute path: " << fs::canonical(fs::path("../share/sync")).string() << endl;
    cerr << "absolute path: " << fs::absolute(fs::path(".") / "index.html").parent_path().string() << endl;
    cerr << "absolute path: " << fs::absolute(fs::path("/home/trivigy/Syncaide/Projects/sync/build/Debug/bin") / "index.html").string() << endl;
    cerr << "absolute path: " << fs::absolute(fs::path("/home/trivigy/Syncaide/Projects/sync/build/Debug/bin") / "index.html").parent_path().string() << endl;
    cerr << "absolute path: " << fs::absolute(fs::path("index.html")).string() << endl;
    cerr << "absolute path: " << fs::absolute(fs::path("index.html")).parent_path().string() << endl;
